...
| D4 | Separate test cluster so `verify:live` could enter CI: 🔵 **OPEN, Steve's call, cost attached.** Two symptoms mitigated (orphan count, dead-man); the cause is not. Recommend: defer until the first paying customer, then re-argue under the 5-gate infra policy | `deferred → verify-live-cannot-enter-ci` |
...
| **D8** 🔴 **NEW** | **The coverage gate is declared, passing, and enforced by nothing.** `vitest.config.ts` declares `lines 80 / functions 80 / branches 70 / statements 80`; CI runs `npm test` and never `test:coverage`, so the thresholds are evaluated on no pipeline. Measured 2026-08-20: **91.4% statements · 84.99% branches · 91.03% functions · 92.6% lines, exit 0** — comfortably over, which is exactly why turning it on costs nothing today and everything the day it silently slips. Textbook decorative guard | `vitest.config.ts` vs `.github/workflows/ci.yml`; re-derive with `npm run test:coverage` |
| **D9** 🔴 **NEW** | **The report-only CSP cannot accumulate the evidence its own next step needs.** `next.config.mjs` enforces four directives and ships `Content-Security-Policy-Report-Only` with `script-src` and `report-uri /api/csp-report`. The route writes to **stderr only** — no table, no retention. Vercel runtime-log retention measured 2026-08-20 at ~24h (7-day and 24-hour queries returned identical counts), and `/api/csp-report` did not appear among the 29 distinct request paths in the retained window. So `ratified.csp-enforce-the-safe-four`'s follow-on — enforce `script-src` — has **no data behind it and no way to gather any** | `next.config.mjs`; `src/app/api/csp-report`; Vercel runtime logs |
...
| F-d | J5 retention: renewal receipt + owner-reminder ladder before a heartbeat transition | first arms-length subscription approaching renewal |
...
recorded only in this file is one session away from being lost.** The seven items revision 3 adds —
A0, D8, D9, D10, D11, D12 and E1's unproven half — are not yet in the register, and entering them as
they are ruled on is part of Sprint 3.
