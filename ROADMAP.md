...
| D17 NEW | 🟡 **PARTLY SELF-CORRECTED — measured 2026-08-31:** `.claude/sprint-state.json` now reads branch `master`, not `sprint/2026-08-20-5`. The rest of the row stands. ~~Original:~~ `.claude/sprint-state.json` is stale (branch `sprint/2026-08-20-5`; "rev 3 uncommitted"; owes runs that have run) — rewrite it. (`docs/backlog.md` — queue exhausted, boxes ticked against a renamed runbook — was bannered `SUPERSEDED BY ROADMAP.md §3` in this revision)| claude | no | the next `/sprint` lands on §3 | — | both files, read 2026-08-27 |
