...
**Done when:** `gates.d3-restore-drill` carries a `met:` block — ⚠️ it now carries a `preflight:` block, which is **not** the same claim and does not close the gate; both production clusters are ACTIVE
