...
| **G4's Claude lane ran 2026-09-13**, 25 days before its window: the restore drill **with a decrypt** (`gates.d3-restore-drill.met`: restore 154 s, one real item unwrapped through the product's own route against the scratch cluster, torn down, preflight green), the DR copy-absence alarm seen red and reaching the inbox, the backup wall scheduled under its own audited OIDC role (first run red on a real defect, then green/red proven), CMK rotation on with the pin flipped in the same commit, the budget measured (exists at $250 portfolio-wide; its mail is in Trash), failover R14.2 deferred with a trigger, C3 parked to Sitting E, the old admin key's deletion dated 09-27. What G4 still owes is the ladder — real dates only. | `PROJECT.yaml` `ratified.sitting-d2-2026-09-13.g4_*` · PRs #89, #90 · runs 34773753600 / 34773781289 / 34774159337 |
...
| A7.0 the instrument | ✅ **CLOSED 2026-09-02** — collection was enabled by Steve on 2026-09-01 hours before the op-ed send, `verify:funnel` printed "collection is enabled" that night, and it was re-measured 2026-09-03T03Z: `FUNNEL_COLLECTION=enabled npm run verify:funnel` exit 0, all 7 emit checks. Register entry closed with `revisit_outcome`; the 09-12 revisit is answered. ~~Original: 🔴 **RE-MEASURED 2026-08-31 AND STILL DEAD** — the Vercel Web Analytics API answers `web_analytics_not_enabled` for `prj_VYyOXbTh…`. ⚠️ **And `verify:funnel` passed 7/7 at that same moment while printing "the instrument is alive"** — a claim about both halves from evidence about one. Fixed: the walk now exits **2** (could not look) by default and **1** when collection is known off, and says *"DO NOT LAUNCH A PLACEMENT ON THIS"*. | steve (one toggle) |
...
- ⚠️ **Nothing in production has ever been rotated**; no cadence exists; the `autospecai` admin key
  is 424 days old. (B18)
- ~~⚠️ The DR vault's copy jobs have **no absence alarm** (only failures alert).~~ ✅ `relay-dr-copy-absent` since 2026-09-13. (B17)
...
| A11 NEW | ✅ **MOOT, measured 2026-09-13**: the $2,000 was paid by PayPal on 2026-09-03 and the AWS credit code arrived 2026-08-17 (Gmail). Redemption of the code is Steve's to confirm. Original: Devpost prize delivery ($2,000 + $2,000 AWS credits): if nothing has arrived by ~2026-10-08 (claim completed 2026-08-09 + 60 days), chase `prizes@devpost.com` | steve | yes | the credits are what keeps the infra free | ~2026-10-08 (memory `project_h0_hackathon_orbis_relay`) | memory only — not in the repo |
...
| **B17** ✅ **CLOSED 2026-09-13** (`relay-dr-copy-absent`, seen ALARM → inbox → OK; primary alarm's scope read) — was: | **DR vault copy jobs have no absence alarm** (`relay-backup-absent` watches primary BACKUP completions only; a dropped copy action raises no `COPY_JOB_FAILED`). Decide → add `AWS/Backup NumberOfCopyJobsCompleted` alarm to NotifyMe, `TreatMissingData breaching`, forced to ALARM once; re-read the live `relay-backup-absent` definition (never done). Bundle into the D3 admin session | steve (decision, ~$0.10/mo, 5-gate); claude (build) | ruling + admin session | cross-Region backup coverage matches the runbook's claim | — | `docs/backup-restore-runbook.md §Still open` |
| **B18** NEW 📝 | ✅ **FIRST ROTATION DONE 2026-09-13** — the 441-day `autospecai` admin key, by script under the 5-gate (runbook §5b; new key active, old INACTIVE, walls green twice; Steve deletes the inactive key after a fortnight). 25 keys remain unrotated, by design without a deadline. Original: **Secret rotation: no cadence, nothing ever rotated** (all 26 production keys `createdAt == updatedAt`; six app secrets + the `relay-runtime` key 2026-06-24; `autospecai` admin key 2025-06-29, **424 days**). B18.0 ratify a cadence **or** record "none until first customer" · B18.1 rotate the `autospecai` admin key (two-keys-valid overlap; Claude executes on request; no secret in chat) · B18.2 `relay-runtime` key pair in Vercel (IAM half by CLI, Vercel half by `vercel env`, redeploy = PR, re-run the three walls) · B18.3 the six app secrets each in its own window (NEXTAUTH = global sign-out; JWT secrets only when no release is open; `RESEND_API_KEY`'s provider half is Steve's dashboard; **`STRIPE_*` excluded — shared account, never rolled per project**) · B18.4 confirm `DSQL_PASSWORD` is EMPTY in Vercel and `.env.local` (`vercel env ls production`, names only) · B18.5 remove the inert retired `TOTP_SECRET` from Vercel Production (`vercel env rm`, with a nod) | steve (B18.0; dashboards); co-pilot (B18.1–3); claude (B18.4–5) | ruling; credential-touching → 5-gate | the estate's oldest, most powerful credential stops being 14 months old | — (do not invent one) | `docs/secret-rotation-runbook.md` table + §1–§7; `deferred.every-secrets-age-is-unknown.closed` |
| **B19** ✅ **EXECUTED 2026-09-13** (rotation ON, `ROTATION_INTENDED = true` same commit, PR #89) — was: NEW | **CMK auto-rotation** — `ROTATION_INTENDED = false` records the as-provisioned state; nobody has decided. Enable (`aws kms enable-key-rotation` + flip the pin in the same commit + `verify:kms` green; ~$24/yr; symmetric rotation is backwards-compatible) or record "stay off, deliberately" | steve | ruling (infra change) | the finding `kms-wall.ts` alarms on is "nobody decided" | — | `lib/ops/kms-wall.ts` ROTATION_INTENDED comment |
| **B20** NEW 📝 | ✅ **RULED + DONE 2026-09-13** (`ratified.sitting-d2-2026-09-13.d10_b20_iam_template`): the template no longer grants `dsql:DbConnectAdmin`. Was: **`infra/iam-policy.json` template still grants `dsql:DbConnectAdmin` and `kms:DescribeKey`** — re-applying it silently undoes the 2026-08-16 cutover; its Sid does not start with `Dsql`, so the cutover doc's Sid-filtered audit prints `[]`. Rule: bring the template to the live v2 shape (and decide whether runtime keeps DescribeKey); Claude edits + README | steve (ruling); claude (edit) | ruling | a re-provision cannot re-open the wall | — | `infra/README.md`; `docs/aws-setup.md §6` |
| **B21** | ✅ **B21.3 RULED + DONE 2026-09-13 — retention 30 days; `pruneCspReports()` rides the heartbeat cron as housekeeping** (`ratified.sitting-d2-2026-09-13.d16_b21_3_csp_retention`); B21.4 (nonce CSP) stays parked until real traffic. Original: **CSP tail (D9 closed with an open downstream).** B21.1 make `scripts/verify-csp.ts` an npm script on `.env.ro` (one SELECT) · B21.2 read `csp_reports` for WHICH directives/sources once a real traffic window exists, confirming delivery in a real browser first (an empty table has two meanings) · B21.3 retention/pruning ruling (the table never prunes; public unauthenticated writer, 20/min/IP) · B21.4 the nonce + Node-runtime middleware decision (a request-path change to a working system, 5-gate) → build; `security-headers.test.ts` refuses removing `unsafe-inline` without nonces in the same change | claude (1, 2, build); steve (3, 4) | rulings | removing `unsafe-inline`/`unsafe-eval` from the **enforced** `script-src` | event: real traffic | `deferred.the-csp-report-sink-…closed.still_open_downstream`; `scripts/verify-csp.ts` |
| B22 NEW | ✅ **RULED 2026-09-13 — DOCUMENT as design** (`sitting-d2.d3_b22_executor`; live read: no `executor` row); comment on `VALID_ROLES`. Was: **`executor` role is hidden in the UI but accepted by the API** (`VALID_ROLES` includes it; `POST /api/recipients` and `/api/people` accept it) — the exact dropdown-only pattern the estate ruling rejected. Refuse it at the trust boundary (keep the enum for existing rows; pin by test) or record that the role is acceptable and amend Req 3.1 | steve (one-line ruling); claude (build, S) | ruling | truthful trust boundary | — | `lib/people/recipients.ts:55-56`; `src/app/(owner)/circle/PeopleSections.tsx:72` |
| B23 NEW | ✅ **CLOSED 2026-08-31.** Rule 10: `MAX_STANDBY` (10) now refuses the binding in `claimStandbyRole` — the only moment a relationship binds to an identity — counted across both roster tables and every owner, excluding the row being claimed so a reissued invitation is never self-refused, failing LOUD on a broken count rather than reading absence as zero. Rule 4 shaped the refusal itself: the message names no count and no owners, and nothing is written to the owner's audit chain (a cap refusal recorded there would enumerate for the owner exactly what the rule forbids). The wider rule-4 audit ran the same day: every owner-facing query on `recipients`/`verifiers` is owner-scoped (`owner_id = $1` or joined through the owner's own rows); the only cross-owner reads key on `claimed_user_id` and serve the contact's own dashboard, account lifecycle, or ops. A structural grep-guard for future routes was **considered and not shipped** — the T3.2 precedent: a source-scan guard at high false-positive rates is the check people learn to ignore. ~~Original: rule 10 (no `MAX_STANDBY` exists) and rule 4 — "cheap now, expensive to retrofit"~~ | done | no | Sprint 6 | — | `lib/people/claim.ts`; `lib/people/claim.test.ts` (mutation-checked: `>=`→`>` fails the at-cap test) |
| B24 NEW | ✅ **CLOSED.** `npm run incident:evidence` exists and is documented in CLAUDE.md's command block. ~~Original:~~ **Incident runbook step 0 has no operator tool**: nothing scripts `verifyAuditChain()` per owner or captures the evidence bundle (Vercel logs ~24h retention, chain, `scheduler_runs`, `email_send_attempts`). Build it read-only under `.env.ro`; same script doubles as the pre-release chain check| claude | no | the runbook is executable at 3am | — | `docs/security-incident-runbook.md Step 0`; `lib/audit/chain.ts` |
| B25 NEW | ✅ **RULED 2026-09-13 — accept `NEXTAUTH_SECRET` rotation as the bulk path, documented in the runbook; a built control deferred to the first stranger** (`sitting-d2.d17_b25_bulk_revocation`). Was: **No bulk session revocation** (`bumpSessionEpoch` is per user; global sign-out = rotate `NEXTAUTH_SECRET` = global outage that also breaks step-up/WebAuthn/recovery tokens). The runbook says it "deserves its own decision" | steve | ruling (blast radius) | containment without an outage | — | `deferred.no-incident-or-breach-runbook.closed.there_is_no_bulk_session_revocation` |
| B26 NEW | ⏸ **RULED 2026-09-13 — DEFER with F-b** (`sitting-d2.d17_b26_secret_types_q6_q16`). Was: Secret-types design **Q6**: the owner branch of `/api/kms/unwrap` is outside `step-up-guard`'s scope while TOTP seeds (which outlive a session) are stored — classify under step-up or record a decision not to; **Q16** (seed phrases in scope given the estate disclaimer) is a second unruled question | steve (ruling); claude (build) | ruling | the guard's own criterion is met by a seed | — | `docs/secret-types-design.md §6 Q6, §7` |
| B27 NEW | 🟡 **PART DONE — MEASURED 2026-08-31, the premise “all off” is stale.** Secret scanning **enabled**, push protection **enabled**, vulnerability alerts **enabled**. Still off: **validity checks** and **Dependabot security updates** — one `gh api PATCH` each, still wanting the nod. ~~Original:~~ **Repo security settings all off on a PUBLIC repo**: secret scanning, push protection, validity checks, Dependabot alerts (the feed D12's reopen trigger depends on); CI runs no `npm audit`. Enable alerts + scanning (free; `gh api -X PATCH …security_and_analysis…`, `-X PUT …/vulnerability-alerts`) — **alerts only**, no version-update PRs (majors are barred)| claude (with a nod) | nod | structural secret hygiene; an advisory feed | — | `gh api repos/sgharlow/relay --jq .security_and_analysis` |
| B28 NEW | ✅ **CLOSED 2026-09-10** (PRs #69, #73; Sprint 1 row 1.4; `deferred.owner-mode-a11y-is-armed-by-configuration-not-required` closed) — struck 2026-09-12; the row below is the pre-closure text, kept as the record: Owner-mode accessibility is audited by nothing scheduled (`a11y.yml` runs `A11Y_SCOPE=public`; the default owner `demo@relay.test` no longer exists). Standing pre-release cadence: `disposable-owner.ts create` → `a11y-audit.mjs` with `A11Y_OWNER_EMAIL` → `close` → `verify:orphans` (counts against the signup limit). **2026-09-02: the repo side is BUILT** on branch `d21-relay-ro-ci` — OIDC role `relay-ro-ci`, migration 040, owner scope in `a11y.yml` and `A11Y_REQUIRE_OWNER` so the proof-of-red can be red — with the infra pending `/safe-execute` (`docs/d21-runner-db-oidc-proposal.md` §7); until the fixture owner exists owner mode is ARMED BY CONFIGURATION rather than required, tracked at `deferred.owner-mode-a11y-is-armed-by-configuration-not-required` (owner steve, revisit 2026-09-12), so do not read this row as closed on a green run. | claude | no | CC8's 36-page coverage stays true | before each release that touches owner-mode UI | `a11y.yml:16-20,69`; `scripts/a11y-audit.mjs:120-132` |
| B29 ✅ **DONE 2026-09-13** (`.github/workflows/backup-wall.yml`, daily, OIDC role `relay-backup-wall-ci`, green/red proven) — was: | `scripts/backup-status.mjs` — the only instrument that reads **both** vaults — is hand-run only (last recorded 2026-08-20). Decide a schedule: an AWS Backup read-only OIDC role (the `kms-wall` pattern, no stored secret) is the shape | steve (IAM role); co-pilot (runs) | admin session | a dated answer to "how stale is the newest recovery point" before any infra change | — | `scripts/backup-status.mjs:13-32` |
...
| B32 NEW | ✅ **CLOSED 2026-08-31, by PR as the row required.** All six workflows on `node-version: '22'` moved to `'24'` (ci, a11y, cadence-watch, date-guards, kms-wall, production-canary — stripe-contract-monitor was already there), `@types/node` ^20 → ^24, `tsc` clean under the new types. **`engines` stays `>=22` deliberately**: it is a floor, not a test — this laptop develops on 22.23.2 and `>=24` would refuse it; the tested runtime (CI) now equals the shipped runtime (Vercel), which is what the row wanted. The Vercel setting untouched, per the row. ~~Original: Vercel runs Node 24.x; CI, a11y and `engines` test on 22; `@types/node` is ^20 — align CI to 24 by PR~~ | done | no | tested runtime = shipped runtime | — | `grep node-version .github/workflows/*.yml`; the PR's own green CI is the proof it runs on 24 |
...
| B35 🔴 **READ 2026-09-13 via the browser plugin: auto-renew ON, expires 2027-08-07, renewal 2027-07-08 — and NO PAYMENT METHOD ON FILE at Cloudflare, so the renewal would fail; card is Steve's to add** — was: | **Domain renewal clock**: `relaystandby.com` registered 2026-08-08 at Cloudflare, **expires 2027-08-08** (RDAP); auto-renew state and card recorded nowhere. A lapsed domain takes down the site, all mail (`send.`, `hello@`, `dmarc@`) and every link in a family's saved invitation. Confirm auto-renew ON (the on-disk Cloudflare token has **no Registrar scope** — 403 on 2026-08-27 — so this is the dashboard or a re-scoped token); Claude records registrar/expiry in `PROJECT.yaml` + a yearly RDAP read | steve (confirm; co-pilot via Claude-in-Chrome); claude (record) | dashboard | the front door, structurally | 2027-08-08 (registry) | RDAP `rdap.org/domain/relaystandby.com` |
| B36 🟡 **MEASURED 2026-09-13** (`portfolio-monthly` $250 exists on the shared account; six alerts since 08-01, five in Trash → Sitting E un-filters or subscribes NotifyMe) — was: | **No AWS Budget/spend alarm** on acct `461293170793` — a DPU burst from abuse on the nine memory-limited public endpoints, or a forgotten scratch cluster after D3, has no alert. Draft: monthly budget ≈ 2× the unit-economics fixed cost → SNS NotifyMe. Bundle into the D3 admin session | steve (5-gate, admin); claude (draft) | admin session | a cost-side dead-man on the proven SNS path | — | `docs/unit-economics.md`; `gates.d3-restore-drill` criterion 4 |
| B37 NEW | ✅ **CLOSED 2026-08-29 — this row read as open for two days after the work shipped, and §3.5 rendered it "ready" tonight on the strength of it.** Verified live 2026-08-31: `go-live.sh` opens with a 🔴🔴 HISTORICAL — DO NOT RUN banner naming both hazards (step 5 seeds production, step 8 overwrites live secrets), `demo-run.ts` carries the same banner dated the same day with the reason it cannot work (the demo-owner account shape no longer exists), and the `env-example.test.ts` citation annotates itself as historical provenance, not an instruction. ~~Original: banner both HISTORICAL; retire `demo-run.ts` with D25~~ | done | no | one fewer production write path reachable by following in-repo instructions | — | `scripts/go-live.sh:1-10`; `scripts/demo-run.ts:1-8` |
| B38 NEW 📝 | ✅ **RULED + DONE 2026-09-13** — a mail forwarder counts; Cloudflare and Google named on `/privacy`, `subprocessors-are-listed.test.ts` gains an INFRA allowlist (`sitting-d2.d5_b38_privacy_subprocessors`). Was: **`/privacy` "Who else is involved" omits Cloudflare** (Email Routing forwards every `hello@`/`relay@` message = support content; DNS/registrar) **and Google** (Gmail, where that mail and the DMARC reports are read). `subprocessors-are-listed.test.ts` derives from `package.json` and structurally cannot see infra-only processors. Ruling: does a mail forwarder count? Then edit + extend the test with an infra allowlist | steve (one line); claude (edit + test) | ruling | truthful copy; one fewer G3 diligence surprise | — | `src/app/privacy/page.tsx:103-114` |
...
| B41 NEW | ⚠️ **2026-09-13: the pack's default (soften) was NOT taken — it contradicted the explicit 2026-08-20 ruling in `lib/contact.ts`; "one business day" stands; the READER half stays Steve's** (`sitting-d2.d6_b41_support_promise`). **Support inbox watcher** — "within one business day" is promised on three surfaces; `hello@` forwards via Cloudflare catch-all to Steve's personal Gmail, read by one person with no absence cover. C6's `ends_when` ("who watches the inbox") was never answered in its closure. Record the operational answer and the fallback | steve | ruling | the promise on `/about` is operated, not just stated | — | `deferred.support-has-an-address-and-no-commitment` (closed, open tail) |
| B42 NEW | ◐ **RULED 2026-09-13 — align; PENDING: `RESEND_REPLY_TO_ADDRESS` is a Vercel env value, Steve's to set** (`sitting-d2.d6_b42_reply_to`). Reply-To/From mismatch (`hello@` vs `relay@`) flagged as a mild spam signal, left unmade on purpose — decide, or record as accepted (automatable via `vercel env` once ruled; treat as policy-gated) | steve | ruling | — | — | `docs/email-dns-runbook.md §2`; `lib/notify/email.ts resolveReplyTo` |
| B5 | ✅ **B5.1 phase B DEPLOYED AND PROVEN 2026-09-03** — PR #58 (`8174a61`), Vercel production 04:18Z, `verify:reveal` 20/20 against production with the new decrypt path live; register entry `b5s-compatibility-window-has-started-closing` closed. The decrypt path now routes on `kms_context_era` (NULL → legacy, `owner_v1` → context, unknown → refuse), names `KeyId`, and wrap ignores an early `KMS_WRAP_WITH_CONTEXT`. **Phase C = a deploy AND a flip**, reversible by turning the flag off. ~~**KMS `EncryptionContext` (tenant separation at the key, not the app).** Design + rollout docs written (the code is deliberately unbuilt)~~; migration 037 **applied both regions** (re-verified by `verify:schema` 2026-09-02; the migration header now says so). B5.0 Steve lifts the gate (reopens on a customer, a partner's diligence, or **any** change on the wrap/unwrap path) · B5.1 phase B code: read `kms_context_era`, still wrap without context, name `KeyId` on Decrypt — a no-op deploy that makes phase C reversible · B5.2 S4-4 live proof (needs one REAL pre-change row → A0) · B5.3 phase C: flip the wrap-with-context flag in Vercel — **point of no return is the first row written with a context; rollback after C is flag-off, never a code revert**. Must not share a quarter with a multi-Region CMK (B3 Option B) | steve (B5.0, B5.3 approval); claude (B5.1–2) | ruling; flag flip | cross-tenant unwrap refused **by KMS** | event: Sprint 6 | `deferred.tenant-separation-at-the-kms-boundary-is-not-cryptographic.ruled`; `docs/encryption-context-{design,rollout}.md` |
...
| D5 | ✅ **CLOSED 2026-08-31 (PR #45).** Three of four prices hold; GoodTrust's $39 renewal has left their pricing page entirely; Proton Emergency Access added. ~~Original:~~ Competitor-price re-verification (Everplans $99.99, Trustworthy Silver $120, GoodTrust $39 renewing, Everplans-via-VSP $27) + add Proton emergency access deliberately (`COMPETITORS.md` has 0 mentions)| claude | no | a price anchor a partner can be shown | **2026-10-17** (60 days from 2026-08-18, `market.competitors_doc`) | `docs/COMPETITORS.md` header |
...
| D16 NEW | ✅ **CLOSED — MEASURED 2026-08-31: there is nothing to delete.** 0 merged branches on origin, 0 local branches, `origin/master` the only remote ref. ⚠️ The first attempt to “clean up” reported two stale branches and both deletes failed with *remote ref does not exist* — the local remote-tracking cache was stale and `git fetch --prune` cleared it. Measuring a cache is not measuring the remote. ~~Original:~~ Delete 16 fully-merged branches (12 local; 4 on origin: `auto/2026-08-22-continuation`, `docs/b4-…`, `ops/kms-wall-watch`, `ops/verify-live-stamp-…`; `docs/sprint-1-calendar-…` is a stale tracking ref — `git fetch --prune`) — all 0 ahead; no worktrees; empty stash| claude (mention, don't ask) | no | no phantom "unmerged work" | — | `git branch -a --no-merged master` = ∅ |
| D17 NEW | ✅ **CLOSED 2026-08-31 (second rewrite, PR #51).** The 08-29 rewrite described Sprint 0 against revision 4; Sprint 0 has since closed and the roadmap moved to revision 7, so `/sprint` would have resumed into a finished sprint — the exact failure the row's first closure was for, one revision later. Now records the §3.5 Claude-court lane execution, the four rows that were already done when that table was drawn, and what each open remainder waits on. ~~Original: `.claude/sprint-state.json` is stale — rewrite it~~ | done | no | the next `/sprint` lands on §3.5 | — | `.claude/sprint-state.json` header |
| D18 NEW | ✅ **CLOSED 2026-08-29 — a third row tonight that read as open after its work shipped.** Verified 2026-08-31: `unit-economics.md` states the plan as **`pro` — VERIFIED 2026-08-29** with the derivation beside it (`vercel teams ls` / MCP `list_teams`), keeps the struck assumption as the record of which it was, and carries the D18/B40 cost-attribution caveat (the account-level plan hosts several projects). ~~Original: still says the plan is unverified — write the derivation, reconcile the Resend cost~~ | done | no | — | — | `docs/unit-economics.md:23,33,97` |
| D19 NEW | ✅ **CLOSED 2026-08-31.** `npm run check:subscription -- <email>` exists, read-only under `.env.ro`, with its two occasions in its own header and in §5: after any real checkout, and on walk days beside `verify:orphans`. What deliberately did NOT happen: unattended scheduling — that needs a DB credential on a runner (D21, Steve's call, the same wall as B28), and an occasion recorded as monitoring would be the overclaim this file keeps finding. ~~Original: in no chain and scheduled by nothing — give it an occasion~~ | done | no | — | — | `package.json`; `scripts/check-subscription.ts` header |
...
| **D21** | ✅ **role + migration DONE 2026-09-02** (PR #60 `1441eff` repo side, PR #61 `4ff0bcf` role + migration 040 in both regions) — ⚠️ **corrected 2026-09-12: the register entry `the-read-only-identity-is-not-in-the-cloud` is CORRECTLY OPEN.** Its `ends_when` needs one of `verify:schema` / `verify:dogfood` / `verify:orphans` / `flight:snapshot` / `verify:roles` OBSERVED running under the role, and `a11y.yml`'s session mint is none of the five; this row overstated for nine days. Original: via OIDC, not a placed file — Steve ruled "yes, via OIDC" 09-01; PR #60 (`1441eff`) + `/safe-execute`: role `relay-ro-ci` (master-ref-pinned, `dsql:DbConnect` only, no KMS) created, migration 040 grants it `relay_ro` in both regions, `verify:iam` 5/5, `verify:roles` intact. Remaining: the proof-of-red and normal dispatches of `a11y.yml` once a dedicated FIXTURE owner is named (`A11Y_OWNER_EMAIL`), then B28 closes. ~~Original: Place `.env.ro` (`relay_ro`: SELECT-only, **no KMS**, reads PII) in the Claude Code cloud environment / CI **and observe one of the five verifications running there** ("wired is not enough"). Prefer the no-stored-secret shape if one exists for DSQL (the `kms-wall` OIDC pattern). Precondition: B16.1 | steve (placement); claude (observed run) | credential placement | unattended agents verify schema/dogfood/orphans/funnel/roles | — | `deferred.the-read-only-identity-is-not-in-the-cloud` |
| D22 NEW | ✅ **CLOSED 2026-08-29.** CLAUDE.md carries *“`/api/health` is 404, on purpose — do not fix it”* with its derivation command. ~~Original:~~ `/api/health` is 404 **by design** (the health surface is `/api/health/scheduler` and `/api/health/delivery-webhook`; a bare 200 is the signal `canary.ts` argues is worthless). Record the intentional absence in CLAUDE.md and point the portfolio live-probe / `/verify` convention at the scheduler route — no new route| claude | no | — | — | `lib/ops/canary.ts:18-21` |
| D23 NEW | ✅ **RULED 2026-09-13** — the review asks: is report-bridge still the precedence, and did G1 move (`sitting-d2.u15_d23_review_on_10_21`). Was: `ratified.relay-resumed-2026-08-21.review_on: 2026-10-21` — carried from the park entry; what it reviews is unstated. State it (precedence? runway?) or remove with a record. Also fix the two entries' time-base date contradiction with a note, not a silent edit | steve (what); claude (record) | one line | — | 2026-10-21 (`review_on`) | `ratified.relay-resumed-2026-08-21` |
...
| D27 NEW | ✅ **CLOSED 2026-08-29 in `PROJECT.yaml` — a fourth row that read as open after the register already held its closure.** Both debts were measured rather than filed on the sentence that raised them: retention is ACCEPTED with the number (4 rows total, 0.30/day, ~108/year, revisit trigger written), and the raw-CSS half is FILED with a live instance found in the filing (`SidebarNav.tsx:124`). ~~Original: file or record as accepted~~ | done | no | — | — | `PROJECT.yaml:4770-4800` (`closed: 2026-08-29`) |
...
| E1.6 | ✅ **CLOSED 2026-08-29 (PR #21).** Both reads recorded in `docs/stripe-setup.md`. ~~Original:~~ Reads #1 (endpoint carries all four events incl. `invoice.payment_failed`) and #2 (default portal cancels **at period end** → `/terms` "cancel … to stop the next one" is true) were answered **read-only via the Stripe CLI on 2026-08-27** — record both in `docs/stripe-setup.md` and close its 🔴/🟡 sections (rev 3 and the go-live checklist wrongly said "no CLI answers these")| claude | no | — | — | `stripe webhook_endpoints retrieve … --live`; `stripe billing_portal configurations list --live` (`bpc_1Tu3WlGs40KMmT4X7RYjcEXF`) |
...
| E1.8 NEW | ✅ **DONE 2026-09-10** — re-paired by Steve; new expiry **2026-12-10**; `verify:stripe` reads `STRIPE_READONLY_KEY` on the scheduled path (Sprint 1 row 1.8). Original: **Stripe CLI session keys (live + test) expire 2026-10-07** — the only non-dashboard read path; re-pairing (`stripe login`) is Steve's browser. Six days after the paywall revisit | steve | yes | E1.2/E1.3/E1.7 lose their read/trigger path | **2026-10-07** (`~/.config/stripe/config.toml`) | config read 2026-08-27 (values not printed) |
...
| E4.4 | ✅ **CLOSED 2026-08-31 (PR #43).** The revisit dead-man judges `decision_due: 2026-10-01`, and a dated cadence must now classify itself. ~~Original:~~ Make the 2026-10-01 revisit **fire** — same mechanism as A0.dm| claude (after A0.dm) | no | — | — | `lib/ops/gates.test.ts:475` asserts presence only |
...
| E7 ✅ **RULED 09-13 (ENABLE) and MERGED the same evening on Steve's approval (PR #85, `automatic_tax` on every checkout session)** — was: | **Sales-tax / Stripe Tax posture is unruled** — $119/yr charged to US consumers by a sole proprietor; no doc mentions tax. Claude reads `stripe tax settings retrieve --live` first; Steve rules (register for Stripe Tax / collect nothing until a threshold / "accepted until first revenue" with a revisit). Shared account — never enable without the other products' say | steve (ruling); claude (read) | ruling | an honest G4 draft | before G4 | `grep -i 'stripe tax'` = 0 hits |
...
| G9 NEW | ✅ **DISPOSITIONED 2026-09-13** (`sitting-d2.u1…u6`): §18 defer → first regulated-partner conversation · §19 defer → G3 signature · §20 defer → G5 · §21 STRIKE · §22 defer → G4 · `business`/`travel` KEEP. Was: **Un-ruled spec planks** the Build Spec banner names: §18 incapacity verification (is N-of-M human verifiers the decided end state?); §19 compliance programmes (SOC 2, GDPR/CCPA, **a DPA template** — the first two questions a regulated G3 partner asks; plus **E7 tax**, which §19 never named); §20 legible trust (bug bounty, whitepaper); §21 open standard; §22 ops (SLOs, a **failover drill** — the H0 failover has never been re-exercised); §23/§24 editions — and `business` / `travel` remain **user-selectable trigger types during a caregiver-only demand test** | each a disposition (rule out / defer with trigger) | steve |
| G10 NEW | ⏸ **RULED 2026-09-13 — defer into §2-F under F-o** (`sitting-d2.u8_req_13_6_plan_preview`). Was: Req 13.6 owner-facing plan preview/annotation — not built, not retired, not ruled | ruling | steve |
| G11 NEW | ✅ **RULED 2026-09-13** — A1 quorums stay separate (A2's coercion objection recorded); B1 challenge-window table ratified as shipped (`sitting-d2.u10_g11_quorums_and_challenge_window`; brief `docs/g11-quorum-and-challenge-window-options.md`). Was: Design decisions with no owner ruling: recovery and release quorums sharing one social graph (Part VI); owner-challenge window per trigger type (2h emergency is a proposal) | Claude drafts options; Steve rules | steve |
| G12 NEW | ✅ **RULED 2026-09-13 — STRIKE until partner pull** (`sitting-d2.u7_mobile_g12`). Was: **Mobile (§23)** — moved from the old G4 row under the 2026-09-01 ruling, content unchanged: survives with no trigger, which under §1 makes it unschedulable; assign one (partner pull / first N recipients on mobile) or strike | ruling | steve |
...
be read, closing the "why does delivery work" question). ✅ **All three done by 2026-09-10** (A7.0 09-02; E1.7 minted, key in use 09-10; the extension drove Sitting A 09-10 and the filter was read 08-30) — struck 2026-09-12.
...
| 0.5 | 🟡 **PART DONE — re-measured 2026-08-31.** ✅ **B15.2 walked 2026-08-30 and again 08-31** (30/30: abstain leaves a 2-of-2 OPEN, one denial halts it back to `armed` with the counter cleared, an unconfirmed verifier's denial is `not_counted` with the quorum ledger untouched). ✅ **B15.3 walked on BOTH paths 08-31** — read-mode 22/22 (08-30 and 08-31) and **cron-mode 22/22**, the latter observing a real production tick (01:00:51Z → 02:00:47Z) escalate a request nobody called. Both halves report `received_confirmations=0`; a lapse is never promoted to consent. They are different code with different failure modes — the read half's call sits behind a swallowing catch, so its failure is silent — which is why one does not prove the other. ⚠️ **B15.1 CANNOT be closed by a walk** — `sweepCheckinReminders` writes its audit row only on successful delivery and `lib/notify/email.ts` refuses reserved TLDs, so a disposable owner satisfies neither; `owner_checkin_reminder_*` still has **zero rows ever**, and the real proof is dated **2026-09-21** (the live owner's 75% rung; ⚠️ re-dated 2026-09-10 to ~10-03, see §3 Sprint 2). Remaining here: **B15.6** — ⚠️ **and it may already be satisfied; nobody can tell from this repo.** Its only definition is one clause in the §2 B15 row citing *"the PR #8 step-up error-classification fix"*, and nothing else here mentions it. The fail-closed property IS proven, and on 08-31 it was extended to `requireStepUpOnce` (account closure — the irreversible path had no fault injection while the reversible one did), proven by planting a fail-OPEN catch. **Restate B15.6 as the property it wants, or confirm this is it** — an item defined only by an identifier in another system gets re-investigated by every session that reaches it. — ⚠️ and this line said "B15.5/B15.6" for about twenty minutes on 08-31 before being corrected: **B15.5 was BUILT on 08-30** (§P1.3 says so, six hundred lines up). Repeating the exact drift this row was being edited to fix is the argument for deriving rather than restating. ~~live proofs:~~ J5-R4 ladder walk (disposable owner, short interval, hourly ticks); verifier deny/abstain/halt walk; J6 4c escalation walk; `verifier-context.ts` action fix; step-up fault injection. Schedule around the signup ceiling (D14): one chain per hour, `verify:live` first (D1) | M | write disposable production rows — tell Steve first; `verify:orphans` after each |
...
- **Sprint 2 (09-17 → 09-24) — real people in the loop.** The cohort's first sends and claims, and
  **B15.1, the first check-in reminder this product has ever sent** — ⚠️ re-dated 2026-09-10: it was
  due 09-21 15:49Z, and Sitting A's owner sign-in moved it to **~2026-10-03**, out of this sprint
  (`npm run check:ladder` prints the date; ANY owner sign-in or change moves it). No walk can
...
| 1.2 | ◐ **A0.2 (clause a) — PART DONE 2026-09-10 (Sitting A).** A real recipient and a real verifier are named and the aliases removed, so `beta:status` is alias-clean for the first time. Both codes were issued on the owner channel from `/circle` ("I will tell them myself", which records the arm) rather than via `phase0-invite.ts`. The gmail rule was re-created after the removal cascaded it. **Left: the verifier claims and passes the four-word call.** The revisit moved to **09-17**, deliberately unacknowledged. Original row: ~~**the REAL circle.** Tonight, 2026-09-10 (was the 09-12 sitting):~~ name one real recipient and one real verifier (neither a plus-alias or dot-variant of the owner), issue each a code via `scripts/phase0-invite.ts` (**never** `invite:cohort` for people already in `/circle`), the four-word verification call after each claims. People named here must **not** also appear in `.relay-cohort.json` | steve (Claude drives via Claude-in-Chrome if wanted) | `beta:status`: April + Ben, both ⚠️ owner-alias, `code issued — not yet` | `beta:status` prints no alias warning and one verifier `confirmed`; `adding-a-person-to-the-circle-does-not-invite-them` carries `revisit_outcome:` |
| 1.3 | ✅ **A0.2b (clause b) — DONE 2026-09-10, PR #65, live-proven both halves.** `readRosterState(raw, everInvited)` (derived, never stored — the column is untouched); `/api/circle` returns `ever_invited`; the `/circle` light says *Not asked yet — nobody has been sent anything. Give them their code*; `beta:status` prints the same. Ten assertions watched red before the fix; `beta:status` against production prints the true sentence for both roster rows; `verify:ui` 43/43 with a new PART 4 assertion on production data. ~~Add a distinct *not-yet-asked* reading to `standby-state.ts` and the `/circle` screen~~ | done | `readStandbyState(null)` → `invited`; no state value for *not asked* | ✅ register `clause_b_built_2026_09_10` |
| 1.4 | ✅ **B28 — DONE 2026-09-10 (PRs #69, #73).** The fixture owner exists, and `A11Y_OWNER_EMAIL` plus the newly required `DSQL_SECONDARY_ENDPOINT` are set. Proof-of-red 34561924459 was red for the right reason (access denied); then green on 34562079649 (9 owner pages clean). `a11y.yml` now fails on master when a value is unset. The first red run, 34560241176, was a configuration failure and is not counted. Original row: ~~**B28 fixture owner — tonight, 2026-09-10.**~~ One dedicated audit-fixture owner account (never a customer — axe prints element HTML into a public log), then `gh secret set A11Y_OWNER_EMAIL`; Claude runs the proof-of-red dispatch and flips `a11y.yml` to fail (not warn) on master when the secret is absent | steve (5 min) → claude | `owner-mode-a11y-is-armed-by-configuration-not-required` open, revisit 09-12, **fires 09-13** | secret set; dispatch red then green; entry closed or its revisit moved with a reason |
| 1.5 | ✅ **D1 + D2 + D19 — DONE 2026-09-10.** `verify:live` 5/5 walks (17+14+43+20+17) stamped 17:25Z @ `d8843e2`; `verify:journeys` 3/3 (27+33+23) stamped 17:28Z, on a restarted dev instance for the signup budget; `verify:orphans` clean; `check:subscription` → paid/active. Dead-men now fire **2026-09-24 17:25Z** (live) and **2026-10-01 17:28Z** (journeys). ⚠️ First attempt hit the wedged-`next dev` trap: a leftover instance on the port answered sign-in with 500 (Jest worker exception on the nextauth route) while the fresh one died EADDRINUSE — kill by PID from `netstat`, confirm ONE listener | done | live stamp 09-03T03:19Z; journeys 08-29T07:53Z | ✅ both stamps 09-10 |
| 1.6 | ✅ **R6 — the Outlook sentence — DONE 2026-09-10.** `docs/beta-cohort-handoff.md` (the `arm` field) and `docs/first-invitations.md` (step 3 and the what-to-expect list) now say: Outlook/Hotmail/Live/MSN invitees go on the **owner arm**; an email-arm code to one of them lands in Junk and reports success; if used anyway, warn them first and ask for the drag to Inbox. Cites Resend's 09-03 answer by date. C2.2/C2.3 were struck in §2-C in revision 8 | done | outcome log had no reply row; C2.2 still open | ✅ both docs carry it |
| 1.7 | ⏸ **A3.1 roster deferred to 2026-09-17 (Steve, 2026-09-10): the dated fourth deferral**, recorded as `ratified.beta-cohort-deferred-four-days.fourth_deferral_2026_09_10` with its own ringing `revisit:`. The circle had to be real first, and it now is. Original row: **A3.1 → A3.4 — the cohort.** A3.1 roster ~~**tonight, 2026-09-10**~~ (Steve, ~10–20 people, both types, both arms; **Outlook addresses on the owner arm**) · A3.2 dry run (Claude, any day) · A3.3 `--commit` on GO (co-pilot, owner cookie) · A3.4 owner-arm sends (Steve, by design) | steve / co-pilot | `.relay-cohort.json`: 1 person, 08-18, never run; N = 0 since 08-12 | `.relay-cohort-codes.json` exists, or a **dated** fourth deferral is recorded on `ratified.beta-cohort-deferred-four-days` |
| 1.8 | ✅ **E1.8 `stripe login` — DONE 2026-09-10 (Steve).** The CLI config was rewritten at 20:52 that night by the login, and the new pairing expires **2026-12-10** (was 10-07). `verify:stripe` holds and now reads via `STRIPE_READONLY_KEY`, the schedulable path, so the CLI pairing is no longer what that check depends on. Original row: ~~tonight, 2026-09-10 (both CLI session keys expire 2026-10-07)~~ | steve (2 min) | `~/.config/stripe/config.toml` expiry 10-07 | `stripe config --list` shows a fresh expiry |
| 1.9 | ✅ **Safe Browsing close-out — DONE 2026-09-10.** Enhanced protection ON (Steve's read); `/vault/new` loaded clean in his Chrome; the INCIDENT entry was closed (PR #69), with why it cleared recorded as unknown. Original row: ~~**Safe Browsing close-out — tonight, 2026-09-10.**~~ Read `chrome://settings/security`; if Enhanced protection is ON and `/vault/new` still loads clean, close the INCIDENT entry with the two facts; if Standard, record that and keep it open | steve (1 min) | loads clean 09-10; protection level unread | entry closed or its `ends_when` re-dated with a reason |
...
| 2.5 | ✅ **BUILT (PR #81) and RUN ONCE 2026-09-13 — both new lines fired; the case body IS entered; the silence is after its first line, in branches that reported via `console.error`, which has never reached a `next start` log. Next instrument built the same day (guards → stderr; `sendOnce` logs entry); one more authorised run names the branch. E1′ stays `wired`.** Original: **E1′ — the handler's first line.** Route 3 after PR #54 returned 200 and *no* loud branch spoke, so the `invoice.payment_failed` case body is never entered. Instrument the handler's first line; one authorised route-3 run (a fresh nod — it writes one fabricated-invoice row to the production audit log); record which branch answers | claude (build) → co-pilot (one run, nod) | E1′ `wired`; six + one deliveries, zero rows | the run prints its branch; the register's `attempted_3` names it — whatever it is |
...
> ⏱ **Revision 9:** this is the **quiet window** — no owner sign-in, no owner write — so row 3.1 (the final rung) does not land here either; both rungs fire in the plan's G4. Row 3.3 is already ruled (E4.1 KEEP INITIATE-ONLY, 08-30) **and its `/terms` sentence has been live since that day — the register entry closed 2026-09-13**. Row 3.4's default stays **extend with a dated revisit**. ✅ **2026-09-13, Claude's half ran early:** row 3.2's instrument proven with a pre-send baseline (`docs/sprint-reports/2026-09-13-phase0-baseline.md`), row 3.5's E7 read done (Stripe Tax active at the account, not applied at the Relay checkout), `verify:journeys` re-stamped (dead-man 10-04). Left: Sitting E's five dashboard reads and the E4.2/E5/E7 rulings — Steve's, recordable early. **And the plan's G3 (10-01 → 10-08) also ran early, 2026-09-13:** the IAM wall is scheduled under its own audited OIDC role (green and red seen), the admin key is rotated, the Devpost prize turned out paid on 09-03. What G3 still owes is the quiet window itself and the daily `check:ladder` read.

| # | Item | Court | Measured 2026-09-10 | Done when |
|---|---|---|---|---|
| 3.1 | **The 90% rung** — ~~2026-09-26 03:49Z~~ **re-derive with `npm run check:ladder`** (09-12: 2026-10-08 03:36Z, after the 75% rung ~10-03; BOTH move with any owner write — `deferred.the-ladder-cannot-be-proven-while-the-owner-is-active`, ruling due 09-17) — second reminder observed the same way as 2.2; then the owner checks in and the ladder clears (the first live ARMED-and-reminded → cleared cycle) | claude (observe) · steve (check in **after** the final rung) | — | `owner_checkin_reminder_final` row; `check:ladder` clean after the check-in |
...
| 3.4 | ✅ **RULED EARLY 2026-09-13 — EXTEND; beta stays free; `decision_due` moved to 2026-11-15** (`ratified.beta-free-release.revisited`; `revisit-dates.test.ts` pins the new date). 10-01 and 10-02 pass with nothing to remember. Original: **E4.2 the 10-01 decision**: flip, or **extend with a new dated revisit**. Recommended default: **extend** — E1′ is `wired` (an expired card would become a silently blocked release), N is days old, and `/terms` promises a notice the code cannot yet prove it sends. If flip: E1.3 route 2 first (co-pilot, test clock, disposable owner), then E4.3's one-commit change-set | steve (ruling) → claude | `ratified.beta-free-release.decision_due: 2026-10-01`; **CI red 10-02 if nothing is recorded** | a `revisited:` block on `beta-free-release` with either a new date or the flip PR |
| 3.5 | ~~E1.8~~ moved to Sprint 1 row 1.8 · **E5** ✅ **RULED 2026-09-13: NO** — `verify:stripe` stays on its own daily workflow · **E7** ✅ **RULED 2026-09-13: ENABLE `automatic_tax` on the Relay checkout** (read first: Stripe Tax active at the account, head office AZ, nothing applied at checkout) — built the same day as a co-pilot PR that waits for Steve's approval, because it changes what every customer is charged · originals: **E5** does any billing check join an automated chain (ruling) · **E7** Stripe Tax posture (Claude reads `stripe tax settings retrieve --live` first; Steve rules) | steve | E1.8 expiry 10-07 | both rulings recorded, or parked with dates |
...
> ⚠️ **Numbering (noted 2026-09-12):** Sprints 4–8 below keep revision 7's numbering; Sprints 1–3 above are revision 8's. Sprint 4 follows revision-8 Sprint 3 in the calendar (→ 2026-11-08) — `docs/gap-closure-plan-2026-09-12.md` §4 G4 carries the same window.

### Sprint 4 — Recovery proven *(calendar: → 2026-11-08 · co-pilot · one admin session · dated by `gates.d3-restore-drill.due`, a ceiling)*

| # | Item | Blocks on | Court |
|---|---|---|---|
| 4.1 | ✅ **DONE 2026-09-13 — `gates.d3-restore-drill.met`**: restore 154 s, one real item decrypted through `/api/kms/unwrap` on a local production build against the scratch cluster, scratch cluster gone, preflight green after; next ceiling 2026-12-13. Was: **D3.0–D3.4** the restore drill end to end **including a decrypt**; `met:` recorded on the gate with the observed RTO. 🟡 **PRE-FLIGHTED 2026-08-31 — READY** (`npm run drill:preflight`, recorded on the gate): criterion 2 holds (`verify:kms` green), criterion 3's precondition holds (1 real decryptable item), and criterion 4's end state already holds (both clusters ACTIVE, deletion protection on, backups 21.2h fresh and byte-identical across Regions). ⚠️ **Margin is THIN** — one item means no second attempt, so a failed unwrap could not be told from an item that was always broken; one extra vault write before the drill removes that ambiguity. Remaining: **criterion 1, the scratch cluster** — spend + infrastructure, Steve's. `npm run drill:plan` prints the phases, ARNs and the three traps. | **A0** ✅ satisfied; spend approval | co-pilot |
| 4.2 | ✅ **B17 DONE 2026-09-13** (`relay-dr-copy-absent` seen ALARM → inbox → OK; `relay-backup-absent` scope READ: primary only) · 🟡 **B36 MEASURED** (a $250 portfolio budget already exists on the shared account; its mail lands in Trash → Sitting E) · ✅ **B18.1 DONE 09-13** (G3) · ✅ **B29 DONE** (`backup-wall.yml` daily under `relay-backup-wall-ci`; green 34773753600, red 34773781289). Was: Same admin session: **B17** DR copy absence alarm (forced to ALARM once) + re-read `relay-backup-absent` · **B36** AWS budget → SNS · **B18.1** rotate the `autospecai` key · ~~**B16.5** delete `relay-backend-dsql` if present~~ (**closed by measurement 2026-08-29 — the role is ABSENT**; nothing to delete) · **B29** decide the backup-status schedule | rulings from Sprint 1.4 | co-pilot |
| 4.3 | **B12** the off-GitHub heartbeat, if ruled — built on the SNS path this session proves again | B11.1 evidence + ruling | co-pilot |
| 4.4 | ✅ **B19 EXECUTED 2026-09-13** (PR #89; `verify:kms` red on the pin first, green after) · **B18.2/B18.3** runtime key and app-secret rotations in their own windows, if a cadence was ratified; **B19** `enable-key-rotation` + the pin flip, if ruled on | B18.0 / B19 | co-pilot |
| 4.5 | ~~**D5** competitor prices re-verified + Proton added~~ — ✅ **DONE 2026-08-31**, seven weeks early. Everplans $99.99, Trustworthy Silver $120 and Everplans-via-VSP $27 all hold. 🔴 **GoodTrust's $39 renewal is no longer on their pricing page** — it now shows a flat $149 "Estate Plan", *"one price, the only Estate plan you'll ever need"*, with **no renewal price at all**; the $39 survives only in a help article, and a price that is only in a help centre is not a market price. **Proton Emergency Access added** — up to 5 contacts, owner-chosen wait of 30/14/7/3/2/1 days or none, owner can deny during the wait, auto-granted on silence, all paid plans. That is a dead-man's switch with an owner-veto window, the closest mechanism any rival ships, and this file did not mention it. | — | done |

✅ **MET 2026-09-13** (the `met:` block is there; both clusters ACTIVE + protected; scratch gone; B17 seen ALARM; the key rotated in G3). **Done when:** `gates.d3-restore-drill` carries a `met:` block — ⚠️ it ~~now~~ carried a `preflight:` block, which is **not** the same claim and does not close the gate; both production clusters are ACTIVE
...
## 3.5. Who does what — every open activity, by court

> *A **rendering**, written 2026-08-31. §2's inventory and its `Court` column win; where this
> section and a row disagree, the row is right and this has a defect. It exists because the split
> was previously readable only by reading twenty tables, and nobody reads twenty tables.*

> 🔴 **DRAWN AFTER A RECONCILIATION, BECAUSE IT HAD TO BE.** Ten §2 rows read as OPEN tonight and
> were not: **D5, E4.4, E1.1, B14, B24, D22, E1.6, D16** were done (four of them closed by *this
> session*, in the sprint tables, without the inventory being updated — my drift, the same
> one-authoritative-place failure this file spends its length on), **B27** is three-fifths done, and
> **D17** has partly corrected itself. A court split drawn from the inventory an hour ago would have
> put eight finished items in somebody's queue. They are marked above; this is drawn after.

### A · Claude's court — nothing needed from Steve

> 🔴 **FOUR OF THESE TWELVE WERE ALREADY DONE WHEN THIS TABLE WAS DRAWN — measured 2026-08-31,
> the day after it shipped.** A1/B13's proofs-of-red ran on 08-29 (the run ids are in the row and
> in the canary's own header); A3/B37's banners shipped 08-29; A6/D27 closed 08-29 in
> `PROJECT.yaml`; A10/D18's doc was corrected 08-29. This table was drawn *after* a reconciliation
> that found ten §2 rows in exactly this state, and still carried four more — because it was
> drawn from the §2 rows, and the §2 rows were the thing that was stale. The correction landed in
> §2 first and this rendering second, in that order, which is the order the banner above demands.

| | Item | State |
|---|---|---|
| **A1** | **B13** proof-of-red for `production-canary` | ✅ was already done 08-29 (runs `33237399208` / `33237979699` / `33237980971`); the date-guards half is **ruled wait** — its planted-gate proof is blocked by the pre-push hook reading the same suite |
| **A2** | **B32** align Node: CI 22 → 24, `@types/node` ^20 → ^24 | ✅ done 2026-08-31; `engines >=22` kept deliberately as the local floor |
| **A3** | **B37** `go-live.sh` / `demo-run.ts` banners | ✅ was already done 08-29; banners verified live 08-31 |
| **A4** | **C1.4** correct the DMARC item everywhere it is written wrong | ✅ done 2026-08-31 — the last drift was the restoration itself, asserted dead in one doc and restored in another |
| **A5** | **D19** give `check-subscription.ts` an occasion | ✅ done 2026-08-31 — `npm run check:subscription`, `.env.ro`, occasions in §5; unattended scheduling still needs D21 |
| **A6** | **D27** unfiled debt | ✅ was already closed 08-29 in `PROJECT.yaml` — measured, not just filed |
| **A7** | **B23** standby cap + cross-owner confidentiality, §3.7 rules 10 and 4 | ✅ done 2026-08-31 — `MAX_STANDBY` refuses at the binding moment; the refusal itself is shaped by rule 4 |
| **A8** | **D17** re-derive `.claude/sprint-state.json` | ✅ done 2026-08-31 |
| **A9** | **D2** `verify:orphans` after every walk day | standing — ran 2026-08-31, see §5 |
| **A10** | **D18** `docs/unit-economics.md` Vercel plan | ✅ was already done 08-29 — the doc says `pro`, VERIFIED, with the derivation |
| **A11** | **C1.3** read the rescued 08-17 Microsoft report — do the A/B IPs show pass/pass? | attempted 2026-08-31 — see the C1.3 row for what the connector can and cannot read |
| **A12** | **D12** major-version currency (next-auth v4, React 18 under Next 16) | ⛔ **recorded, not scheduled.** The Infrastructure Change Policy bars a major upgrade without a documented problem, and there is none. Its reopen trigger is a Dependabot advisory — which is why B27's remaining half matters |

**Blocked, and on exactly one thing each:**

| | Item | Waiting on |
|---|---|---|
| **B27** | validity checks + Dependabot security updates | your **nod** (one `gh api PATCH` each) |
| **B28** | owner-mode a11y in CI | ~~**D21** — a database credential on a runner, your call~~ ✅ D21 done 2026-09-03; now waits on **your fixture owner** in `A11Y_OWNER_EMAIL` (never a customer — axe prints element HTML to a public log), then the proof-of-red dispatch |
| **E4.3** | the paywall flip change-set, pre-written and verified | **E4.2**, your ruling, from 2026-10-01 |
| **B5.1** | ✅ done 2026-09-03 — phase B deployed (#58, `8174a61`) and proven by `verify:reveal` 20/20 on production | — (was: B5.0, lifted 09-01) |
| **D1** | the five-walk chain | the 09-12 window; re-running now spends the 10/hour signup budget to reset a clock with days on it |
| **B11.2** | the cadence re-measure | ✅ done 2026-09-02 — still collapsed after the reset (7/24h on both monitors, 09-03T02Z); cause recorded in `deferred.the-scheduled-monitors-are-collapsing.revisit_outcome`; B12.i is INSTALLED and closed (see B12) — `relay-heartbeat` verified alive 2026-09-09: 684 stamps, last `ok` 2026-09-10T04:09Z, task Ready, LastTaskResult 0 |
| **E3** | the in-product renewal receipt | an **arms-length subscription approaching renewal** — there is none, and the only live one renews 2027-08-09 |
| **B15.1** | the reminder ladder's first firing | **~2026-10-03** (was 09-21; moved by the 09-10 sitting's owner sign-in), the live owner's 75% rung. No walk can substitute — the sweep writes its row only on delivery and the mailer refuses reserved TLDs |

### B · Your court — and *why* each one is manual

*Grouped by reason, because “manual” without a reason is an invitation to automate something that
must not be.*

**B1 · Only a human may do it** — identity, platform ToS, an outbound relationship.
`A1.1`/`A1.7` the op-ed voice pass and send (including the §1a edit the checker found) · `A2` The
Caregiver Space · `A3.4`/`A3.5` the cohort arm, manual **by design** · `A6.3` the three G3 first
contacts · `A10`, `A11` · `A0.2` the invitations nobody has been sent · `C4` A2P carrier
registration. **Claude drafts; a machine-written approach gets the address blocked at exactly these
organisations.**
...
~~`E1.8` `stripe login` **before 2026-10-07**~~ ✅ done 2026-09-10, expiry 2026-12-10 (row 1.8) — was: when both CLI session keys expire and the billing
...
**B4 · Spend or infrastructure** — the 5-gate policy, snapshot and rollback first.
`D3` the restore drill's scratch cluster (three of four criteria already verified and on the gate) ·
`B17` the DR-copy absence alarm · `B36` an AWS budget alarm · `B12` the off-GitHub heartbeat ·
`B18.1` rotate the `autospecai` key · `B3` Option B, a multi-Region CMK, **not in the same quarter
as B5** · `G2` audit and pen test · `G6` entity and insurance — *“funded from the opportunity, not
ahead of it.”*
...
sentence landed early, deliberately; the flip has not · **B29**, **B19**, **D14**, **D4**.

### C · Co-pilot — one session, both present

`E1.2`/`E1.3` the E1′ routes — ⚠️ **route 2 is on hold**: it exercises the same handler body route 3
showed does not write, so it would spend a Stripe test clock and a disposable owner to re-learn what
a spliced POST already shows · `B15.5`/`B15.6` · `B18.2`/`B18.3` · `B20`, `B21.3`/`B21.4` (the CSP
next rung — **not takeable**: 22 inline violations mean nonces or hashes first, which is build work,
not a header flip) · `B38`, `B39`, `B40`, `B41`, `B42`.

### D · Event-gated — nobody can start, and here is what fires each

| Sprint | Fires when | Prepared |
|---|---|---|
...
4. ~~**Do not sign in as the owner between 09-17 and 09-21**~~ — ⚠️ **re-dated 2026-09-10:** the sitting itself needed an owner sign-in and edits, so the first reminder this product has ever sent now fires **~2026-10-03** (`npm run check:ladder`). ANY owner sign-in or change before then moves it again, including confirming the verifier (A0.2 clause a). The cheapest item on this list is still not doing something.
...
**Claude-court rows opened by the beta sprints (§3, revision 8), named here so the court guard sees them:** ~~row **1.3** (A0.2b)~~ ✅ done 09-10 (PR #65) · ~~row **1.5** (D1/D2/D19 chains)~~ ✅ done 09-10 (PR #67) · ~~row **1.6** (the Outlook sentence)~~ ✅ done 09-10 (PR #66) · row **2.2** (observe B15.1) · row **2.5** (E1′ first-line instrument, build half) · row **3.1** (observe the 90% rung) · row **3.2** (A3.6 report).
...
| ~~2026-09-02~~ | ~~B11.1 re-measure the cadence~~ — ✅ **DONE 2026-09-02**: the frequency-selective pattern PERSISTS after the reset (7/24h on both monitors), cause recorded, B12 is the fix | claude | `deferred.the-scheduled-monitors-are-collapsing.revisit_outcome` |
...
| **2026-09-17** | A0.2's moved revisit — has the verifier claimed and passed the four-word call? `adding-a-person-to-the-circle-does-not-invite-them`; **and** the cohort roster's fourth deferral `ratified.beta-cohort-deferred-four-days.fourth_deferral_2026_09_10`; **and** the ladder-window ruling `the-ladder-cannot-be-proven-while-the-owner-is-active`. All three red **2026-09-18** if unanswered (added 2026-09-12; the calendar was not re-derived after Sitting A) | steve → claude (record) | `TZ=UTC npx vitest run lib/ops/revisit-dates.test.ts` |
| ~~**2026-09-17 03:19Z**~~ | ~~`verify:live` freshness dead-man fires~~ — **ran 2026-09-10 17:25Z**; fires **2026-09-24 17:25Z** | claude | `tail -1 docs/verify-live-runs.jsonl` + 14 d |
...
| ~~**2026-09-19 07:53Z**~~ | ~~`verify:journeys` freshness dead-man fires~~ — **ran 2026-09-10 17:28Z**; fires **2026-10-01 17:28Z** | claude | `tail -1 docs/verify-journeys-runs.jsonl` + 21 d |
| ~~2026-09-21 15:49Z~~ **~2026-10-03** | **B15.1 — the first check-in reminder ever sent** (75% rung, live owner). Moved 09-10 by Sitting A; moves again with any owner sign-in or change | claude (observe) | `npm run check:ladder` |
| **2026-09-22** | G3 follow-ups: Homethrive, Wellthy, NAC | steve | `docs/g3-outreach-log.md` |
| ~~**2026-09-26 03:49Z**~~ re-derive | the 90% rung (final reminder) for the live owner — `npm run check:ladder` (09-12: 2026-10-08 03:36Z; it and the 75% rung move with ANY owner write) | claude (observe) | `npm run check:ladder` |
...
| ~~**2026-10-07**~~ **2026-12-10** | Stripe CLI session keys expire — `stripe login` (re-paired 2026-09-10; the scheduled path reads `STRIPE_READONLY_KEY`) | steve | `~/.config/stripe/config.toml` |
...
- `verify:orphans` after every walk day or interrupted fixture run — and `npm run
  check:subscription -- <owner email>` beside it on walk days (D19, 2026-08-31: the billing read
  happens on the days somebody is already reading production).
...
- `npm run check:subscription` after any real checkout (D19 — wired as a script 2026-08-31,
  read-only under `.env.ro`).
...
- [x] **A0** ✅ done 2026-08-29 (`verify:dogfood` READY) — Walk `docs/vault-checklist-sprint-1.md` at relaystandby.com: two real items (one login
...
- [x] **C1.2** ~~Untrash the three reports now in Trash~~ **NOTHING IS IN TRASH — re-measured a
      third time 2026-08-31 with `includeTrash: true`**: the 08-17 Microsoft/relaystandby report
      (message `1a00e72c9417ac7b`) is retained under the DMARC label with no TRASH label. The 08-29
      re-measure said the same; this row kept describing the state anyway, which is the §4
      calendar's DMARC/Trash defect surviving in a third place. **No untrash exists to do and no
      purge clock is running.** *Replaced by:* **C1.3's last hop** — download the 08-17 report's
      `.xml.gz` attachment (~30s in the Gmail UI; the connector has no attachment tool and a
      RAW-MIME read failed its gzip CRC), then Claude gunzips and answers the A/B question:
      do `54.240.48.188` / `54.240.11.161` show pass/pass, disposition none?
- [x] ~~**B10** Remove the Gmail rule~~ — struck 2026-09-12: the filter was read 08-30 and has NO delete action (`b10_prime`); the residue (relay mail still lands in Trash, mechanism unnamed) is on the entry's `re_scoped_2026_09_12`, owner claude. Was: Remove the Gmail rule that **B10.d** (Sprint 0) names as sending `[sgharlow/relay] Run
...
- [x] **C1.0** ✅ closed 2026-08-30 (`dmarc-had-no-rua-so-no-report-could-arrive`) — Was `rua=` removed from `_dmarc.relaystandby.com` on purpose? If yes, say so and it
...
- [ ] **B15.4** Ratify rungs 75% / 90% and the 12 h cooling-off; rule on owner self-naming (default:
      refuse the row)
- [x] **B11.2 / B12** ✅ 2026-09-02: the cadence did NOT recover with the reset (7/24h on both
      monitors), so the money half is moot — B12.i interim BUILT 09-01 (PR #53), install pending
      (Steve, elevated). The AWS half (Route 53 / CloudWatch Synthetics → SNS NotifyMe, 5-gate)
      remains a ruling; no date is forced by this
...
- [x] ✅ **A7.0 — done 2026-09-01/02** (the toggle was thrown; closure recorded on `the-funnel-walk-spoke-for-a-half-it-could-not-see`) — ~~🔴 MEASURED 2026-08-29, AND IT IS WORSE THAN "OFF". ENABLE WEB ANALYTICS.~~
...
- [x] 🔑 **E1.7 — ✅ minted; `verify:stripe` reads `STRIPE_READONLY_KEY` since 2026-09-10** — was: mint a RESTRICTED read-only Stripe key. Stripe dashboard → Developers → API
...
- [ ] **C2.1** Open the Resend support ticket (drafted; Claude first strips the two now-false
      DMARC sentences)
- [x] ~~**C2.2**~~ **STRUCK 2026-09-10 as moot** (Resend 09-03: Outlook = domain reputation, no remedy but volume — do not re-open) — was: Submit the Microsoft sender-support form under your Microsoft account (Claude may
