...
| | Item | State |
|---|---|---|
| **A1** | **B13** proof-of-red for `production-canary` — never red in 685 runs | ready |
| **A2** | **B32** align Node: Vercel runs 24.x, CI/a11y/`engines` on 22, `@types/node` ^20 | ready |
| **A3** | **B37** `scripts/go-live.sh` would seed `demo@relay.test` into production — banner or retire | ready |
| **A4** | **C1.4** correct the DMARC item everywhere it is written wrong | ready |
| **A5** | **D19** `check-subscription.ts` is in no chain and scheduled by nothing — give it an occasion | ready |
| **A6** | **D27** unfiled debt carried five sprints (`email_send_attempts` retention; scrollable-regions) | ready |
| **A7** | **B23** standby cap (`MAX_STANDBY`) + cross-owner confidentiality, §3.7 rules 10 and 4 | ready |
| **A8** | **D17** re-derive `.claude/sprint-state.json` | ready, small |
| **A9** | **D2** `verify:orphans` after every walk day | standing |
| **A10** | **D18** `docs/unit-economics.md` still calls the Vercel plan unverified — it is **`pro`** | ready, small |
| **A11** | **C1.3** read the rescued 08-17 Microsoft report — do the A/B IPs show pass/pass? | ready; needs the report to hand |
