...
| 2026-09-02 | Step 1 | ✅ **SENT 2026-09-03T02:48:17Z** from Steve's Gmail to `support@resend.com` (thread `1a0650dde420bf2a`). Resend's auto-acknowledgement arrived 27 seconds later ("We've got your message… primary support hours 2 AM–5 PM Pacific"). Measured in Gmail Sent, not assumed from the draft disappearing. C2.1 is closed; the reply, when it comes, is a row here. |
...
| 2026-09-03 | Step 1 | 🔴 **ANSWERED 2026-09-03T16:00:26Z by Resend (Customer Success), same thread — and the reply sat unrecorded here for seven days while the row above still said "the reply, when it comes, is a row here".** Verbatim on the substance: *"no, we don't raise mitigation requests with Microsoft, and filing one against those IPs wouldn't help you here, because there isn't an IP problem to mitigate. Microsoft also doesn't run a sender program equivalent to Gmail Postmaster Tools … SCL 5 is the score Microsoft assigns mail it has no opinion about. It isn't a penalty. It means relaystandby.com has neither positive nor negative reputation with them, and with nothing to weigh, the mail settles in Junk. That's also why all four IPs scored the same: the score is tracking your domain, not the infrastructure it left on. At under ten emails a day there's very little for Microsoft to form a view from. What changes the outcome is steady volume and engagement on the domain over time … Recipients opening your mail, replying to it, and moving it from Junk to Inbox is the strongest signal available to you."* Links given: resend.com/blog/how-to-warm-up-a-new-domain · resend.com/docs/knowledge-base/warming-up. Recorded 2026-09-10 (ROADMAP revision 8). |
| 2026-09-10 | Step 3 | **STRUCK AS MOOT on the answer above.** The Microsoft form asks for mitigation of an IP or a penalty; there is neither. Not parked, not "blocked on the account" any more — retired. What replaces it: the beta-cohort handoff tells the owner to put outlook.com/hotmail/live invitees on the **owner arm** and to warn email-arm invitees to look in Junk (ROADMAP §3 Sprint 1 row 1.6). The one re-test worth doing is opportunistic — after the cohort has generated a few weeks of real sends and replies, one fresh-mailbox SCL read, recorded as the domain's first reputation reading (ROADMAP C2.3 as struck). See memory `feedback-outlook-scl5-is-no-reputation-not-a-penalty`: do not re-open. |
