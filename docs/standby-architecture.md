...
| 11 | **Elevation is a ROW, never a claim** — added 2026-08-15, shipped. Re-authenticating for a sensitive action mints a nonce in `auth_challenges`; every check consults it, and it is bound to subject *and* session epoch. | Rule 1's reasoning, one layer up. A signed cookie proves we minted it and **cannot be withdrawn before it expires** — so "sign out everywhere" would otherwise leave a five-minute window open on the device somebody has just declared compromised. The actions it guards (a fresh recovery sheet, a plaintext export, closing the account) are the ones that OUTLIVE the session that performed them, which is what makes a 24-hour session the wrong authority for them. ⚠️ The guard **stands down** for an account holding neither TOTP nor a passkey — a freshly-claimed contact has neither, and a guard nobody can satisfy is a lockout, not a defence. |
| 12 | **An authentication nonce is spendable once, and only the database can say so** — added 2026-08-15, shipped. Every WebAuthn ceremony burns its challenge. | **A signature proves we minted a token; it never proves the token is unspent.** The seal was a stateless five-minute JWT and nothing burned it, so a captured assertion replayed for the rest of that window. The same distinction is why rule 11 is a row: both are cases where a valid-looking credential says nothing about whether it has already been used. |
