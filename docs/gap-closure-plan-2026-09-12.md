...
| **GP-P5** | Backup/restore with a decrypt — `gates.d3-restore-drill` | ✅ **`met:` recorded 2026-09-13** (was: preflight READY 08-31; due 2026-11-08) | One admin session per `docs/backup-restore-runbook.md`, incl. one KMS unwrap of a restored row | CP + S (spend, admin) | G4 | `met:` recorded with the row id and unwrap result |
...
| **GP-D11** 5-gate | B17 / B36 / B29 | DR copy, AWS spend, backup status: no absence alarm | ✅ **09-13:** B17 alarm live and seen red; B29 `backup-wall.yml` green/red; B36 measured (exists, mail in Trash → Sitting E). Was: Sitting H bundle; each additive with delete-to-rollback; the confirmed SNS topic `NotifyMe` (us-east-1, acct 461293170793) already exists | S → C | G4 | each seen red once |
...
| G0.2 | **GP-R1, R3, R4** — nod-gated register edits (task 5.1 second half) — ✅ **nodded and done 2026-09-13** (R1 closed with clause 3 recorded as partial; R4 has its `ends_when`; R3 stays correctly open) | S nod → C | done |
...
| G0.10 | **GP-P6** — ✅ **confirmed and ticked 2026-09-13** (`b15_6_confirmed_by_steve_2026_09_13`) | S line → C | done |
| G0.11 | **GP-U13 → GP-D2** — ✅ **nodded and built 2026-09-13**: `lib/audit/action-labels.ts` (every emitted action, bound both ways by `lib/ops/audit-actions-are-labelled.test.ts`, which derives the emitted set from the source), visible raw sub-line on `/audit` | S nod → C | both-way test green; owner-mode a11y audit result in the G1 report |
| G0.12 | **GP-D15** — ◐ nodded 2026-09-13: Dependabot security updates confirmed ON; secret-scanning **validity checks are not settable via the API** (PATCH accepted, value stays disabled) — the UI toggle is Steve's, ~1 min | S nod → C → S | one of two on |
| G0.13 | **GP-U9** — ✅ **RULED 2026-09-13: (a) accept the natural date** (`sitting-d2.gp_u9_ladder_window`); the entry is closed on the ruling; the proof closes the ladder entry | S ruling | done |
...
| G1.5 | **GP-P2** E1′ — ✅ instrument built (PR #81) and **the authorised run happened 2026-09-13: both lines fired, the case body IS entered, nothing after its first line spoke or wrote** — the 09-02 hypothesis is refuted and the silence is now bounded to branches that used `console.error`, which has never reached a `next start` log. Next instrument built the same day (guards → stderr; `sendOnce` logs entry); **one more authorised run** names the branch. E1′ stays `wired` | C → CP | the branch named — not yet |
| G1.6 | **A6 follow-ups 09-22** logged, including silence | S | three outcome rows |
| G1.7 | **Sitting D-2** — ✅ **RULED 2026-09-13 by ratified default** (`ratified.sitting-d2-2026-09-13`, twenty rows). Two defaults NOT taken and recorded as such: B41 (contradicted the explicit 08-20 ruling; "one business day" stands) and B42 (a Vercel env value, Steve's). B19 executes in G4 | S | done |
| G1.8 | The edits the rulings license — ✅ **shipped 2026-09-13 in one PR**: `/privacy` names Cloudflare + Google (test extended); `VALID_ROLES` comment (B22); IAM template matches the live policy (B20); runbook sentence (B25); CSP retention prune on the cron (B21.3); challenge-window ratification comment (G11 B1); R14.1 evidence recorded. Not shipped: Reply-To (Steve's env), copy softening (default not taken) | C | merged |
...
### G4 — The ladder fires; recovery proven *(2026-10-08 → 11-08 · = rev-7 Sprint 4 / Sitting H · co-pilot · one admin session ≈ 2–3 h)* — **Claude's lane ran 2026-09-13 under the goal "use cli/automation where possible": the drill, the three alarms and B19 are done or measured; the ladder rows (G4.1) wait for real dates and nothing else does**
...
| G4.2 | **GP-P5** — ✅ **DONE 2026-09-13**: Steve approved the spend in-session; by script: `backup-now` → restore 154 s → scratch cluster → local production build → one real item through `POST /api/kms/unwrap` → decrypted (shape only, never printed) → scratch deleted → `drill:preflight` green. `gates.d3-restore-drill.met`; next ceiling 2026-12-13 | C (was CP + S) | `met:` recorded ✅ |
| G4.3 | **GP-U11(b)** — ✅ **RULED 2026-09-13: DEFERRED with a trigger** (next drill 12-13 or G1 pass); entry closed on both halves | S ruled | R14.2 dated or deferred ✅ |
| G4.4 | **GP-D11** — ✅ **B17** `relay-dr-copy-absent` seen ALARM → inbox 17:40:53Z → OK, `relay-backup-absent` scope read (primary only); ✅ **B29** `backup-wall.yml` daily under `relay-backup-wall-ci` (first run RED on a real defect, PR #90 fix; green 34773753600 / red 34773781289); 🟡 **B36** measured — a $250 portfolio budget already exists on the shared account, six alerts since 08-01, five in Trash → **Sitting E** (revisit 09-19) | C (was S 5-gate → C) | two seen red ✅; B36 to Steve's mailbox |
| G4.5 | **B19** — ✅ **EXECUTED 2026-09-13** (PR #89): `EnableKeyRotation` by the Node SDK + `ROTATION_INTENDED = true` in one commit; `verify:kms` RED on the pin first, GREEN after; daily `kms-wall.yml` green | C | daily wall green ✅ |
| G4.6 | **GP-D12 C3** — 🅿️ **PARKED 2026-09-13** to after Sitting E's report read (09-19); Steve changes `p=` in the Cloudflare UI | S | dated ✅ (09-19) |
...
**State 2026-09-13 (run early, before Sprint 2):** everything in G4 that is not the ladder is done, ruled or measured — G4.2 met, G4.3 ruled, G4.4 two of three seen red and the third measured, G4.5 executed, G4.6 dated, G4.7 untouched (B5.0's reopen conditions are not met; the 10-21 review is a calendar event). **Still Steve's:** the ladder (G4.1, real dates only), the budget mail filter (Sitting E), the DMARC step-up after the read, deleting the inactive admin key on 09-27. **Stop rule, trailing three sprints (register rows closed / gate outcomes):** G2 — 1 closed + 2 ruled early; G3 — 2 closed; G4 — 2 closed (B17, U11) + the D3 gate met + B29/B19 done + B36 measured. Every sprint cleared the 3-row minimum; the Claude lane of this plan is now exhausted and the next scoping note is Steve's calendar, not a sprint.
