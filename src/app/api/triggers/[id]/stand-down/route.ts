 ...
 * is introduced.
 *
 * It used to be the counterpart to /cancel, and the important difference was
 * that this one is recoverable: CANCELLED is terminal — nothing transitions out
 * of it — so cancelling a false alarm retired the access rule permanently.
 *
 * ⚠️ THERE IS NO LONGER ANYTHING TO BE THE COUNTERPART OF. /cancel and its
 * button were retired on 2026-08-21: two rounds of copy fixes had gone into
 * making a two-tap control honest about permanently retiring a trigger type, and
 * the ruling was that no wording earns that on the screen somebody opens in a
 * hurry. This is now the only stop control, which raises rather than lowers the
 * stakes on the liveness defect recorded below — if standing down stops working,
 * an owner has no way at all to halt a false alarm short of checking in.
 *
