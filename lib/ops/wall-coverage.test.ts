...
// … and `relay-backup-wall-ci` (2026-09-13, G4.4): it reads cluster STATE and vault listings, never rows.
const NO_DATABASE_HALF = ['relay-kms-wall-ci', 'relay-iam-wall-ci', 'relay-backup-wall-ci'];
...
describe('the database wall and the IAM wall watch the same identities', () => {
  ...
  it('the ONLY principal excluded from that comparison is the one with no database half', () => {
    ...
    */
    ...
    expect(NO_DATABASE_HALF).toEqual(['relay-kms-wall-ci', 'relay-iam-wall-ci', 'relay-backup-wall-ci']);
  });
